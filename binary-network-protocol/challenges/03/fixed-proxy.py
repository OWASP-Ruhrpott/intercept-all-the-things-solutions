...
def getNonce(message):
    nonce = message[7:10]
    print("\033[91m[!]\033[00m Nonce:", nonce)
    return nonce

def modify(message):
    print("\033[91m[!]\033[00m Start modification of request...")
    print("\033[91m[!]\033[00m Request:", message)
    #### START IMPLEMENTING YOUR LOGIC HERE

    print(message[0])

    if message[0:1] != b"\x42":

        nonce = getNonce(message)
        userbytes = b"\x00\x00"+nonce
        password = "admin"

        message = getMessage(userbytes, password)
